...
const AutomatedTradingControl: React.FC = () => {
  ...
  const startEngine = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setError(errorData.detail || 'Failed to start trading engine');
      }
    } catch (error) {
      setError('Error starting trading engine');
  ...
  const stopEngine = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const errorData = await response.json();
        setError(errorData.detail || 'Failed to stop trading engine');
      }
    } catch (error) {
      setError('Error stopping trading engine');
  ...
  const executeManualAnalysis = async () => {
    ...
    try {
      const response = await fetch(`/api/automated-trading/execute-manual-analysis?symbol=${manualSymbol.toUpperCase()}`, {
        method: 'POST'
      });
      ...
      if (response.ok) {
        ...
        setError(null);
        await loadRecentActivity(); // Refresh to show new analysis
        setManualSymbol('');
        
        // Show success message
        alert(`Analysis completed for ${result.symbol}: ${result.decision.action} (${(result.decision.confidence * 100).toFixed(1)}% confidence)`);
      } else {
        const errorData = await response.json();
        setError(errorData.detail || 'Failed to execute manual analysis');
      }
    } catch (error) {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-2xl font-bold">Automated AI Trading Engine</h2>
        </div>
        <Button onClick={() => { loadEngineStatus(); loadRecentActivity(); }} variant="outline">
      ...
      {engineStatus && (
        <Card>
          <CardHeader>
            <CardTitle className="text-lg">Monitored Symbols</CardTitle>
...
export default AutomatedTradingControl;
